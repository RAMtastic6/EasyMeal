...
export default function ReservationUser({ params }: { params: { id: string } }) {
    ...
    async function fetchInitalData() {
      ...
      try {
        ...
        if (result1.state === 'accept' || result1.state === 'to_pay' || result1.state === 'completed') {
          ...
          const payStatus = await checkOrdersPayStatus({ reservation_id: parseInt(params.id) });
          setIsPaid(payStatus);
        }
        if (result1.state === 'to_pay') {
          const session = await verifySession();
          if (result1.isRomanBill) {
            const price = await getRomanBill({ reservation_id: parseInt(params.id), customer_id: session.id });
            setIndividualPrice(price);
          }
          else {
            const price = await getPartialBill({ reservation_id: parseInt(params.id), customer_id: session.id });
            setIndividualPrice(price);
          }
        }
      } catch (error) {
  ...
  const handlePayment = async () => {
    const response = await pay({ reservation_id: parseInt(params.id) });
    setIsPaid(true);
    if (!response) {
